        ...
        "environment": {
            ...
            "properties": {
                "id": {
                    "type": "string",
                    "description":
                        "ID of the test environment\n"
                        "\n"
                        "Must start with a non-empty string identifying the "
                        "CI system owning the test environment, followed by "
                        "a colon ':' character.\n"
                        "\n"
                        "The rest of the string is generated by the owner "
                        "CI system, and should identify the environment "
                        "uniquely and consistently among all environments "
                        "owned by that system, appearing in reports. "
                        "E.g. could be an FQDN of the host the test ran on.",
                    "pattern": f"^{ORIGIN_ID_PATTERN}$",
                    "examples": [
                        "redhat:ibm-p8-kvm-09-guest-10."
...
def inherit(data):
    ...
    Returns:
        The inherited data.
    """
    # We can handle it, pylint: disable=too-many-branches
    ...
    for test in data.get('tests', []):
        ...
        if 'environment' in test:
            ...
            if environment:
                # Generate an ID from JSON representation
                environment['id'] = "_:" + hashlib.sha256(
                ...
                # Discard empty environments
                del test['environment']
